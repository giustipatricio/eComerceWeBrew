...
if($_POST){
  ...
  $name = $_POST['name'];
  try {
    $query = $baseDeDatos->prepare ("UPDATE segment
        SET
          `segment_id` = '$id',
          `segment_name` = '$name'
          WHERE `segment_id` = '$id';
  ...
  <body>
          <?php require "../NavlAdmin/nav.php" ?>
            ...
            <div class="" id="singin" style="text-align: -webkit-center; padding:0px; margin:0 auto"> </div>
              ...
              <form class="form-horizontal" id="amb" action="../AMB/segmentsmod.php?segment_id=<?=$_GET["segment_id"]?>" method="post" enctype="multipart/form-data">
              <div class="cuadro">
              <div class="titulo" style="text-align-last: center;"><h2>ESTILO A MODIFICAR</h2></label>
                ...
                <br>
                <input style="display:none" name="segment_id" id="segment_id" value="<?=$segment['segment_id'];?>" >
                </table>
                <button type="submit" class="btn" id="botones" name="Submit" value="Enviar" style="width:50%">Modificar Estilo</button>
              </form>
              <br>
              <br>
              <a  class="btn" id="botones"  style="width:50%" href="../AMB/segments.php"> Menu Estilo </a>
            </div>
          </div>
          <div class="espacio" style="padding-top:3vw"> </div>
          </div>
