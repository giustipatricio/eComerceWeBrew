  ...
  <tbody>
    <?php  foreach ($origin as $origen):?>
      <tr>
        ...
        <td><a id="trash"> <i class="fas fa-trash"></i></a></td>
        <div id="<?= $origen['country_id']?>" class="popup-wraper">
        <div class="popup">
          <div class="popup-content">
            <h1>¿Estas seguro de borrar el item?</h1>
            <a class="popup-close">NO</a>
            <a href="<?= url("/eliminar_pais/{$origen->country_id}")?>">
                SI<i class="fas fa-trash" style="color:black;"></i>
            </a>
          </div>
        </div>
      </div>
...
<!-- FIN DE OFERTAS -->
    ...
    <script type="text/javascript" src="{{ URL::asset('js/deleted.js') }}"></script>
